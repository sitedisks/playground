...
class TestSpider(Spider):
    ...
    def parse_details(self, response):
        ...
        try:
            ...
            item['_tel'] = response.xpath(
                '//a[contains(@href, "tel:")]/text()').get().strip()
            item['_fax'] = find_between(
                response.text, '传真：', '联络：').split('<br/>')[0].strip()
            item['_contact'] = find_between(
                response.text, '联络：', '地址：').split('<br/>')[0].strip()
            item['_email'] = find_between(
                response.text, '邮件：', '网站：').split('<br/>')[0].strip()
            website = find_between(response.text, '网站：', '<br/>').strip()
            if len(website) > 1:
